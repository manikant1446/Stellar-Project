...
#![cfg(test)]use super::*;use soroban_sdk::{testutils::Address as _, vec, Env, String\};#[test]fn test_init_and_vote() {    let env = Env::default();    env.mock_all_auths();it_and_vote() {    let env = Env::d\$0    let contract_id = env.register(Contract, ());    let client = ContractClient::new(&env, &contract_id);    let user1 = Address::generate(&env);    let user2 = Address::generate(&env);    // Initialize poll with candidates    client.init(&vec![$0
    client.init(&vec![
        &env,
        String::from_str(&env, "Alice"),
        String::from_str(&env, "Bob"),
